...
export default function DataManage({ userRole }) {
  ...
  const [imports,       setImports]       = useState([]); // grouped import sessions
  const [loading,       setLoading]       = useState(true);
  const [mode,          setMode]          = useState("home"); // home | undo | bulk
  const [selectedIds,   setSelectedIds]   = useState(new Set());
  const [selectedBatch, setSelectedBatch] = useState(null); // import session to undo
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch all audit logs, filter for IMPORT in JS — avoids composite index requirement
  ...
  const fmtDate = (d) => {
    ...
    // YYYY-MM-DD string — split directly to avoid timezone shift
  ...
  // ── Find records imported in a given batch (within 5-min window of audit log entry) ──
  ...
  // ── Undo import: delete all records from that batch ──
  const handleUndoImport = async () => {
    ...
    for (let i = 0; i < batchRecs.length; i++) {
      try {
        await deleteDoc(doc(db, "registrations", batchRecs[i].id));
        count++;
    ...
    await logAudit("BULK_DELETE", {
      reason: "Undo import",
      file: selectedBatch.details?.file,
      count,
      undoneBy: auth.currentUser?.email,
    });
  ...
  const handleBulkDelete = async () => {
    ...
    for (let i = 0; i < ids.length; i++) {
      try {
        await deleteDoc(doc(db, "registrations", ids[i]));
        count++;
      } catch (err) { setError(`Error: ${err.message}`); }
      setProgress(Math.round(((i + 1) / ids.length) * 100));
    }
    await logAudit("BULK_DELETE", {
      reason: "Manual bulk delete",
      count,
    ...
    <div className="app-layout">
      ...
      <div className="sidebar-content">
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"2rem" }}>
          ...
          {mode !== "home" && (
            <button className="btn-secondary" onClick={() => { setMode("home"); setSelectedIds(new Set()); setSelectedBatch(null); }}>
              ← Back
            </button>
          )}
        </div>
        ...
        {deleting && (
          <div className="card" style={{ textAlign:"center", padding:"2.5rem", marginBottom:"1.5rem" }}>
            <p style={{ fontFamily:"'Cinzel',serif", fontSize:"0.82rem", color:"#155c31", letterSpacing:"0.2em", textTransform:"uppercase", marginBottom:"1rem" }}>
              Deleting Records…
            </p>
                ...
                <div style={{ display:"flex", alignItems:"flex-start", gap:"1rem", marginBottom:"1rem" }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize:"0.85rem", color:"#6b7280", lineHeight:1.6 }}>
                      Select a previous import session and delete all records that were imported in that batch.
                    </p>
                  </div>
                </div>
                <button className="btn-primary" onClick={() => setMode("undo")}
                  style={{ background:"#d97706", fontSize:"0.78rem", width:"100%" }}>
                  View Import History
                </button>
              </div>
              ...
              <div className="card" style={{ borderLeft:"4px solid #991b1b" }}>
                <div style={{ display:"flex", alignItems:"flex-start", gap:"1rem", marginBottom:"1rem" }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize:"0.85rem", color:"#6b7280", lineHeight:1.6 }}>
                      Search and select individual records to permanently delete multiple entries at once.
                    </p>
                  </div>
                </div>
                <button onClick={() => setMode("bulk")}
                  style={{ background:"#991b1b", color:"#fff", border:"none", padding:"0.6rem 1rem", fontFamily:"'Cinzel',serif", fontSize:"0.78rem", letterSpacing:"0.1em", textTransform:"uppercase", borderRadius:"4px", cursor:"pointer", width:"100%" }}>
          ...
          <div>
            <div className="card" style={{ marginBottom:"1.5rem", borderLeft:"4px solid #d97706" }}>
              ...
              <p style={{ fontSize:"0.85rem", color:"#374151", marginBottom:"1.25rem" }}>
                Select an import session below to preview the records it added, then confirm deletion.
              </p>
                  ...
                  {imports.map((batch) => {
                    ...
                    return (
                      <div key={batch.id} onClick={() => setSelectedBatch(isSelected ? null : batch)}
                        style={{
                          padding:"1rem 1.25rem", border:`2px solid ${isSelected ? "#d97706" : "#e5e7eb"}`,
                          borderRadius:"6px", cursor:"pointer", background: isSelected ? "#fffbeb" : "#fff",
                          transition:"all 0.15s",
                        }}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                          <div>
                            <p style={{ fontFamily:"'Cinzel',serif", fontSize:"0.8rem", color:"#111827", fontWeight:"600" }}>
                              📁 {batch.details?.file || "Unknown file"}
                            </p>
                            <p style={{ fontSize:"0.8rem", color:"#6b7280", marginTop:"2px" }}>
                              {fmtDate(batch.timestamp)} · by {batch.userEmail} ·{" "}
                              <strong style={{ color:"#155c31" }}>{batch.details?.count || "?"} records imported</strong>
                              {batch.details?.skipped > 0 && <span style={{ color:"#991b1b" }}> · {batch.details.skipped} skipped</span>}
                            </p>
                          </div>
                          <div style={{ display:"flex", alignItems:"center", gap:"0.75rem" }}>
                            <span style={{ fontSize:"0.8rem", color:"#6b7280" }}>
                              {batchRecs.length} records found in DB
                            </span>
            ...
            {/* Preview selected batch */}
            {selectedBatch && (() => {
              const batchRecs = getBatchRecords(selectedBatch);
              return (
                <div className="card" style={{ marginBottom:"1.5rem", borderColor:"#fca5a5" }}>
                  <h3 className="section-head" style={{ color:"#991b1b" }}>
                    ◈ Preview: {batchRecs.length} records will be deleted
                  </h3>
                  <div style={{ overflowX:"auto", maxHeight:"300px", overflowY:"auto", marginBottom:"1rem" }}>
                    <table style={{ width:"100%", borderCollapse:"collapse" }}>
                      <thead>
                        <tr>
                          {["Matai Title","Holder Name","Village","District","Cert No."].map(h => (
                  ...
                  <button onClick={() => setConfirmOpen("undo")}
                    style={{ background:"#991b1b", color:"#fff", border:"none", padding:"0.65rem 2rem", fontFamily:"'Cinzel',serif", fontSize:"0.78rem", letterSpacing:"0.1em", textTransform:"uppercase", borderRadius:"4px", cursor:"pointer" }}>
          ...
          <div>
            ...
            <div className="card" style={{ marginBottom:"1.5rem" }}>
              <div style={{ display:"flex", gap:"1rem", alignItems:"center", marginBottom:"1rem" }}>
                <input
                  type="text" placeholder="Search by title, name or village…"
                ...
                {selectedIds.size > 0 && (
                  <button onClick={() => setConfirmOpen("bulk")}
                    style={{ background:"#991b1b", color:"#fff", border:"none", padding:"0.65rem 1.5rem", fontFamily:"'Cinzel',serif", fontSize:"0.78rem", letterSpacing:"0.1em", textTransform:"uppercase", borderRadius:"4px", cursor:"pointer", whiteSpace:"nowrap" }}>
                    🗑 Delete {selectedIds.size} Selected
                  </button>
                )}
              </div>
              <p style={{ fontSize:"0.82rem", color:"#6b7280" }}>
                {selectedIds.size} of {filteredRecords.length} records selected
                ...
                <table style={{ width:"100%", borderCollapse:"collapse" }}>
                  <thead style={{ position:"sticky", top:0, zIndex:1 }}>
                    <tr>
                      ...
                      {["Matai Title","Holder Name","Village","District","Type","Cert No."].map(h => (
                        <th key={h} style={th}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                    ...
                    {filteredRecords.length === 0 ? (
                      ...
                      return (
                        <tr key={r.id} onClick={() => toggleSelect(r.id)}
                          style={{ background: sel ? "#fef2f2" : i % 2 === 0 ? "#fff" : "#fafafa", cursor:"pointer", transition:"background 0.1s" }}>
                          ...
                          <td style={td}>{r.district || "—"}</td>
                          <td style={td}>
                            <span style={{ background:"#e8f5ed", color:"#155c31", fontSize:"0.72rem", padding:"2px 8px", borderRadius:"10px", fontFamily:"'Cinzel',serif" }}>
                              {r.mataiType || "—"}
                            </span>
                          </td>
        ...
        {confirmOpen && (
          <div style={{
            ...
            <div style={{ background:"#fff", borderRadius:"8px", padding:"2rem 2.5rem", maxWidth:"480px", width:"90%", boxShadow:"0 20px 60px rgba(0,0,0,0.3)" }}>
              ...
              <h3 style={{ fontFamily:"'Cinzel',serif", fontSize:"1rem", color:"#111827", textAlign:"center", marginBottom:"0.75rem" }}>
              ...
              <div style={{ display:"flex", gap:"1rem", justifyContent:"center" }}>
                <button onClick={() => setConfirmOpen(false)} className="btn-secondary" style={{ fontSize:"0.82rem" }}>
                  Cancel
                </button>
                <button
                  onClick={confirmOpen === "undo" ? handleUndoImport : handleBulkDelete}
