...
export default function Certificate({ userRole }) {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      // 5. Add "CONTROLLED COPY" footer text directly onto PDF (outside certificate image)
      pdf.setFontSize(7);
      pdf.setTextColor(160, 160, 160);
      pdf.setFont("helvetica", "normal");
      pdf.text(
        `CONTROLLED COPY — ${newPrintId} — Generated by ${auth.currentUser?.email || "unknown"} on ${new Date().toLocaleString("en-WS")}`,
        pageW / 2,
        pageH - 3,
        { align: "center" }
      );

      // 6. Open in new tab instead of downloading
